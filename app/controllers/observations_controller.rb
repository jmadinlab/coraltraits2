class ObservationsController < ApplicationController
  ...
  def count
    ...
    if params[:model2]
      ...
      if @model1 == "trait" or @model1 == "standard" or @model1 == "methodology"
        ...
        # @observations = @observations.joins(:specie).where(specie: {class: params[:class1]}) if params[:subclass1].present?
        puts "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
        puts params[:class1]
      elsif @model1 == "user"
    ...
    render json: {
      pub: @observations.where(:access => true).size,
      pri: @observations.where(:access => false).size
    }
  end
  ...
  def update_multiple
    ...
    Observation.where(:user_id => params[:con_id], :id => params[:all_ids]).update_all(:access => false)
    ...
    if params[:pub_ids]
      ...
      params[:pub_ids].each do |pub|
        ...
        if @observation.resource
          puts "=======================YESYESYESY=============================="
          @observation.update(:access => true)
        else
          puts "=======================NONONONONO=============================="
          fails << "#{pub}"
  ...
  def index
    ...
    @search = Observation.search do
      ...
      any_of do
        with(:access, true) unless logged_in? && current_user.admin?
        with(:user_id, current_user.id) unless logged_in? && current_user.admin?
      end
    ...
    if params[:all]
      @observations = Observation.all.paginate(page: params[:page], per_page: 9999)
    else
      @observations = Observation.all.paginate(page: params[:page])
    end
  ...
  private
    ...
    def set_observation
      ...
      if @observation.access == false
        if !logged_in? | (logged_in? && @observation.user_id != current_user.id && !current_user.admin?)
