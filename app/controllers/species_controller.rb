class SpeciesController < ApplicationController
  ...
  def create
    ...
    if @specie.specie_name.present?
      begin
        client = Savon.client(
            :wsdl => "https://www.marinespecies.org/aphia.php?p=soap&wsdl=1",
            :open_timeout => 10,
            :read_timeout => 10,
            :log => false
        )
      ...
      @aphia_id = @aphia.body[:get_aphia_id_response]
      @aphia_id = @aphia_id[:return]
    ...
    if @specie.aphia_id.present?
      ...
      @specie_name = @aphia.body[:get_aphia_name_by_id_response]
      @specie_name = @specie_name[:return]
  ...
  private
    ...
    def get_specie_csv
      csv_string = CSV.generate do |csv|
          ...
          @species.all.each do |specie|
            ...
            csv << [specie.id, specie.specie_name, specie.family_molecules, specie.family_morphology, specie.subclass, syn_vec.join(", "), specie.specie_description]
          end
        end
     return csv_string
    end
    ...
    def specie_params
      params.require(:specie).permit(:specie_name, :specie_description, :family_molecules, :family_morphology, :subclass, :aphia_id, :user_id, :approved, synonyms_attributes: [:id, :synonym_name, :synonym_description, :_destroy])
    end
